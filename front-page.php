...
$shopics=get_post_meta($homempid,"shopimages",$single=false);if($shopics){
  echo '<div id="proaside"><div class="container"><div class="row clear">';
  foreach($shopics as $shopic){$shopiclib=explode("###",$shopic);if(@$shopiclib[1]){$shoplink=$shopiclib[1];}else{$shoplink='#';}if($shopiclib[0]){
    echo '<div class="col-xl-4 col-xs-4 col-md-4 col-sm-4 text-center"><a href="'.$shoplink.'" style="background-image:url('.$shopiclib[0].')"><img src="" alt=""/></a></div>';
  }
}echo '</div></div></div>';}?>
